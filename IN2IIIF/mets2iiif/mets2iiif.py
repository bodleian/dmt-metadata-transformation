...
class Mets2iiif(In2iiif):
         ...
         for property in properties:
             if property != '':
                 ...
                 if xpath != "":
                     ...
                     result = doc.xpath(xpath , namespaces={'goobi': 'http://meta.goobi.org/v1.5.1/', 'mets': 'http://www.loc.gov/METS/', "mods": "http://www.loc.gov/mods/v3"}) # use xpath to extract corresponding string or node values from the mets file
                         ...
                         if type(item) == str: 
                             ...
                             if len(language) > 0:
                                     ...
                                     if len(existingValues) == 0:
                                        dictMetadata[property] = [] # if dictionary entry does not exist then create list value
                                         
                                     dict = {'@value': value, '@language': lang}
                                     dictMetadata[property].append(dict)  # update dictionary
                                 
                             else:
                                     existingValue = dictMetadata[property] # if no language attribute exists then use different construct for property value
                                     if existingValue != '': 
         ...
         if (arg.image_src == 'directory') and (os.path.isdir(arg.image_location) == True): # canvases and images defined using files in image directory
             ...
             for item in structMap:   # create canvases using structMap
                ...
                if item.attrib['ORDERLABEL']:
                    ...
                    canvas_label =  self._canvas_label(counter) # or if not present, use canvas label prefix defined in config
     ...
     def _getImageLocation(self, canvas_id):
         ...
         if os.path.isabs(image_location) is False: # if image location is relative then get mets file location from input parameter and append
